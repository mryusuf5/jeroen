<x-admin-layout>
    ...
    @endif
    <a href="{{route('admin.diplomas.create')}}" class="btn btn-info">Foto toevoegen</a>
    <br>
    <br>
    <div class="images row g-0 justify-content-center gap-2">
        @foreach($images as $image)
            <div class="d-flex flex-column gap-2 col-md-3 col-10">
                <img src="{{asset('images/diplomas/' . $image->image_path)}}" class="object-fit-cover" alt="">
                <form action="{{route('admin.diplomas.destroy', $image->id)}}" method="post" class="confirmForm">
                    @csrf
                    @method('DELETE')
